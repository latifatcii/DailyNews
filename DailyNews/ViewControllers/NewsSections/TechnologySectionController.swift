...
class TechnologySectionController : UIViewController , UICollectionViewDelegateFlowLayout , UICollectionViewDataSource {
    ...
    func fetchNews(page : Int) {
        ...
        dispatchGroup.enter()
        FetchTopHeadline.shared.fetchData(THRequest(country: "us", category: .technology, q: nil, pageSize: 10, page: page)) { (result) in
            dispatchGroup.leave()
            switch result {
        ...
        dispatchGroup.enter()
        FetchTopHeadline.shared.fetchData(THRequest(country: "us", category: .technology, q: nil, pageSize: 5, page: 1)) { (result) in
            dispatchGroup.leave()
            switch result {
            case .success(let news):
