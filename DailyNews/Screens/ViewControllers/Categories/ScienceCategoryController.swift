...
class ScienceCategoryController: FeaturedCategoryController {
    ...
    override func fetchNews(page: Int) {
        ...
        dispatchGroup.enter()
        dispatchQueue.async {
            FetchNews.shared.fetchData(THRequest(country: "us", category: .science, qWord: nil, pageSize: 10, page: page)) { [weak self] (result) in
        ...
        dispatchGroup.enter()
        dispatchQueue.async {
            FetchNews.shared.fetchData(THRequest(country: "us", category: .science, qWord: nil, pageSize: 10, page: 1)) { (result) in
                switch result {
                case .success(let news):
