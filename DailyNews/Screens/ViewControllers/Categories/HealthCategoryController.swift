...
class HealthCategoryController: FeaturedCategoryController {
    ...
    override func fetchNews(page: Int) {
        ...
        dispatchGroup.enter()
        dispatchQueue.async {
            FetchNews.shared.fetchTopHeadlineNews(THRequest(country: "us", category: .health, qWord: nil, pageSize: 10, page: page)) { [weak self] (result) in
        ...
        dispatchGroup.enter()
        dispatchQueue.async {
            FetchNews.shared.fetchTopHeadlineNews(THRequest(country: "us", category: .health, qWord: nil, pageSize: 10, page: 1)) { (result) in
                switch result {
                case .success(let news):
